...
ObjProgressbar::ObjProgressbar(LPDIRECT3DDEVICE9 device)
{
	m_device = device;
...
void ObjProgressbar::DrawMain(D3DXMATRIXA16* pmatViewProj, float energy)
{
...
HRESULT ObjProgressbar::_InitVB()
{
	/// 정점버퍼 생성
...
HRESULT ObjProgressbar::_InitIB()
{
	TRI_IDX guage[] =
	{
		{0, 1, 2}, {2, 1, 3}
...
HRESULT ObjProgressbar::_InitMtrlEdge()
{
	D3DMATERIAL9 mtrl;
...
HRESULT ObjProgressbar::_InitMtrlGauge()
{
	D3DMATERIAL9 mtrl;
	ZeroMemory(&mtrl, sizeof(D3DMATERIAL9));
	mtrl.Diffuse.r = mtrl.Ambient.r = 1.0f;
