...
ObjSkyBox::ObjSkyBox(LPDIRECT3DDEVICE9 device)
{
	m_device = device;
}
ObjSkyBox::~ObjSkyBox()
...
HRESULT ObjSkyBox::Create()
{
	...
	for (int i = 0; i < 4; i++)
	{
		D3DXMatrixRotationY(&forTXT[i], PI/2*i);
	}
	D3DXMatrixRotationX(&forTXT[4], -PI / 2);
	D3DXMatrixRotationX(&forTXT[5], PI / 2);
