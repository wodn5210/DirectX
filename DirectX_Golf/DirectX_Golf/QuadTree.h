...
public:
	...
	int GenerateIndex(VOID* pIb, TERRAIN_VTX* pHeightMap, ObjFrustum* pObjFrustum);
...
private:
	...
	int	_GenTriIndex(int nTris, VOID* pIndex, TERRAIN_VTX* pHeightMap, ObjFrustum* pObjFrustum);
	...
	int			_GetLODLevel(TERRAIN_VTX* pHeightMap, D3DXVECTOR3* pCamera)
	{
		//float d = _GetDistance((D3DXVECTOR3*)(pHeightMap + m_nCenter), pCamera);
		float d = _GetDistance(&((pHeightMap + m_nCenter)->p), pCamera);
		return max((int)(d * m_fLODRatio), 1);
	}

	// 현재 노드가 LOD등급으로 볼때  출력이 가능한 노드인가?
	BOOL		_IsVisible(TERRAIN_VTX* pHeightMap, D3DXVECTOR3* pCamera)
	{
		int lv = _GetLODLevel(pHeightMap, pCamera);
