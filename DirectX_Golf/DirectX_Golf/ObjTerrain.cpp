...
HRESULT ObjTerrain::Create(LPDIRECT3DDEVICE9 device, ObjFrustum* pFrustum,  D3DXVECTOR3* scale,
...
HRESULT	ObjTerrain::DrawMain()
{
	VOID* pIndices;
	if (FAILED(m_pIB->Lock(0, (m_x - 1) * (m_z - 1) * 2 * sizeof(TRI_IDX), (void**)& pIndices, 0)))
		return E_FAIL;
	//쿼드 트리에서 인덱스 채우기
	m_nTriangles = m_pQuadTree->GenerateIndex(pIndices, m_pHeightMap, m_pFrustum);
